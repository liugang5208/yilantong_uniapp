	...
	<view class="edit-container">
		...
		<u-navbar :is-back="false" title="" :background="{ background: '#ffffff' }" :border-bottom="false">
			<view class="d_a_sb" style="width: 750rpx;">
				...
				<view style="font-size: 32rpx; color: #1a1a1a; font-weight: 600; text-align: center; flex-grow: 1;">编辑发票资料</view>
		...
		methods: {
			doIninit() {
			    ...
			    that.$api.ticket_info(params).then(ret => {
			      that.info = ret.data;
			      that.tex_type = Number(ret.data.tex_type) || 1;
			      that.ticket_type = Number(ret.data.ticket_type) || 2;
