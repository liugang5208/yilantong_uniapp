<template>
	<view class="wrap custom-class">
		<div class="search d_a_sb">
			<div class="d_a" style="flex: 1;margin-right: 20rpx;">
				<div class="search_loc">
					<image class="search_loc_image" src="/static/icon/ic_loc.png" mode="widthFix" />
					<span>{{label}}</span>
				</div>
				<div class="search_input d_a_sb" @click="gotoSearch()">
					<div style="height: 80rpx;flex: 1;" class="d_a">
						<text>输入产品型号或商品名称</text>
					</div>
					<image src="/static/icon/ic_search.png" class="sh" mode="widthFix" />
				</div>
			</div>
			<div class="search_msg d_a">
			...
			<swiper-item class="swiper-item" v-for="(item, index) in swiperList" :key="item.id">
				<scroll-view v-if="isShow" scroll-y style="height: 100%;width: 100%;" @scrolltolower="onreachBottom">
					<view style="padding: 20rpx;">
						<u-swiper :height="300" name="source_url" :list="banner"></u-swiper>
					</view>
		...
		<!--MASK SLIDE-->
		<div class="menu_mask" @click="closeNav()" v-if="navi_slide>0"></div>
		<div class="menu_body" v-if="navi_slide>0">
			<div class="menu_body_title d_a_j" @click="closeNav()">
				<image src="../../static/imgs/cha.png" style="width: 40rpx;height: 40rpx;" mode=""></image>
				<text style="margin-left: 10rpx;">全部分类</text>
			</div>
			<div :class="x.id==navi?'active':''" class="menu_body_list" v-for="(x,i) in cats" @click="clickNav(i)">{{x.name}}</div>
		</div>
		...
		methods: {
			...
			changeInfo(info) {
				...
				if (info.price_type == "1") {
					uni.navigateTo({
						url: '/pages/shops/shop_more?ids=' + info.id + "&type=" + info.price_type
					})
				}
			},
			slideNav() {
