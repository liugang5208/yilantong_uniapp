...
> 整理日期：2026-09-04（最后更新：2026-09-16）
...
| `pages/welcome/welcome.vue` | 未提交 | 修复开屏视频播放死循环，新增 `onVideoError`，容器改用 `opacity` 控制显隐（见第十五节） |
...
11. 开屏视频修复（第十五节）只做了代码逻辑修复与静态验证，视频自动播放、静音切换、倒计时、播完跳转等实际效果还需要用户在浏览器/真机里肉眼确认；视频加载失败时的兜底是直接跳过进首页，没有重试或降级展示默认图片，是否需要更完善的降级策略需要业务确认
12. `App/Common/Common/function.php::uploadFile()` 根目录不存在时不会自动创建（yilan 后端，见 [`docs/2026-09-16-changelog.md`](../../yilan/docs/2026-09-16-changelog.md) 第七节），本次只手动补齐了本地环境缺失的 6 个上传目录，其它环境如果也缺目录会复现同样的报错链路
